...
{	
    ...
    //private DragonRecognizer _dragonRecognizer;
    ...
    public void onCreate() 
    {
		...
		try 
		{
			...
			recognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 1000 * 4);
			...
			VoiceCommand.showNotify(context, VoiceCommand.NOTIFY_OFF, R.drawable.mic_off);
    ...
    private void setAudioManager()
    {
    	if(audio.isWiredHeadsetOn() 
    			&& !prefs.getBoolean(VoiceCommand.PREF_USEHEADSET, false)){			
    		L.d(this, "routing audio to speaker");
    ...
    public void RecognizerResults(ArrayList<String> al)
    {
    	...
    	try {
    		...
    		L.d(this, "RecognizerResults: time="+ d.getMinutes() + ":" + d.getSeconds());
        ...
        if (keycode == KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE || keycode == KeyEvent.KEYCODE_HEADSETHOOK) {
            ...
            if (action == KeyEvent.ACTION_UP) {
            	...
            	else
            	{
    				...
    				{
    					L.i(this, "Speech off");
    					VoiceCommand.showNotify(context, "Speech recognition off", R.drawable.mic_off);
    					...
    					if(!enabled)
    					{
    						VoiceCommand.showNotify(context,VoiceCommand.NOTIFY_SLEEP , R.drawable.mic_on);
    						enabled=true;
    						
    					}
    					else{
	    					L.i(this, "Speech on");
		    				VoiceCommand.showNotify(context, "Speech recognition on", R.drawable.mic_on);
		    				startUserIndicator();
	...
	private void broadcastStateChange(int state, Object o)
	{
		...
		if(state == VoiceCommand.STATE_ERROR)
			...
			intent.putExtra("result", (String)o);
		context.sendBroadcast(intent);
	}
	...
	private void registerMediaButton()
    {
    	...
    	mRemoteControlResponder = new ComponentName(getPackageName(), EventReceiver.class.getName());
	...
	IVoiceCommandService.Stub api = new IVoiceCommandService.Stub() {
		...
		@Override
		public void removeListener(SpeechResultsListener listener)	throws RemoteException {
		...
		@Override
		public void addListener(int id, SpeechResultsListener listener)	throws RemoteException {		
...
//	{
//		_speechKit = SpeechKit.initialize(getApplication().getApplicationContext(), AppInfo.SpeechKitAppId, AppInfo.SpeechKitServer, AppInfo.SpeechKitPort, AppInfo.SpeechKitApplicationKey);
//        _speechKit.connect();
//	}
	...
	private TimerTask housekeepingTask = new TimerTask(){
		...
		public void run() {
			...
			{
				try 
				{ if (l.listener.isAlive()) continue;} 
				catch (Exception e)
				{ cleanPatterns(l.id); }
			}			
		}};
